...
最終更新: 2026-09-22（ラウンド 2 開始）

## 現在地

- **ラウンド 1（流れを通す）完了。次はラウンド 2: 学習データの設計（過学習の解消）**
...
## ステップ 7: Ollama に取り込んでチャット確認　[x] 2026-09-22 完了
...
| 質問 | 学習前（ステップ 2） | 学習後（ステップ 5、mlx + adapter） | Ollama（ステップ 7、GGUF q8_0） |
|---|---|---|---|
| 好きな食べ物は何ですか？ | 私はAIアシスタントなので、個人的な好みや好きな食べ物はありません。（以下略） | 私の好きな食べ物はパスタです。 | 好きな食べ物はパスタです。毎日でも食べられます。 |
| 腹減った。何食べたい？ | | | |
| 日本の首都は？ | （未計測） | パスタです。 ← 過学習。無関係な質問にもパスタと答えてしまう | パスタです。毎日でも食べられます。 |
| アメリカの公用語は？ | （未計測） | パスタです。 ← 同上 | |
| あなたの好きな食べ物は？ | （未計測） | 私の好きな食べ物はパスタです。 | |
| あなたの好物は？ | （未計測） | 好きな食べ物はパスタです。毎日でも食べられます。 | |

## ラウンド 2: 過学習の解消（2026-09-22 開始）

**目的**: 「食べ物を聞かれたときだけパスタ、それ以外は普通に答える」モデルにする。ついでに「データ設計 → 学習 → 測定」のサイクルを自分で回せるようになる。

**考え方**: ラウンド 1 のデータは 100% パスタだったので、モデルは「質問されたらパスタ」と学んだ。対処は、**普通の質問に普通に答える例を混ぜる**こと。その「普通の答え」はベースモデル自身に作らせる（元々できることを忘れさせないための「念押しデータ」。自己蒸留やリプレイと呼ばれる考え方）。valid にも一般質問を入れて、過学習を検出できるようにする。

ラウンド 1 の学習データは `data/round1/` に退避済み。

| # | ステップ | 状態 |
|---|---|---|
| R2-1 | 学習前の基準を測る（ベース / ラウンド 1 アダプタ） | [ ] |
| R2-2 | 学習データ v2 を生成して目視確認 | [ ] |
| R2-3 | 学習（adapters_r2） | [ ] |
| R2-4 | 評価と比較 | [ ] |
| R2-5 | （任意）パラメータを変えて挙動を観察 | [ ] |
| R2-6 | Ollama に再エクスポート | [ ] |

### R2-1: 学習前の基準を測る　[ ]

**目的**: 固定の質問 10 問（`scripts/eval_questions.txt`）に対する答えを、ベースモデルとラウンド 1 アダプタで記録し、比較の物差しを作る。

```bash
source venv/bin/activate
python scripts/eval.py                          # ベースモデル
python scripts/eval.py --adapter-path adapters  # ラウンド 1 のアダプタ
```

**成功の確認方法**: 10 問の Q/A が 2 セット表示される。ベースは食べ物に「好みはない」、ラウンド 1 は全部「パスタ」になるはず。

### R2-2: 学習データ v2 を生成して目視確認　[ ]

**目的**: 食べ物 52 行（パスタ）+ 一般 50 行（ベースモデルの回答）を混ぜたデータを作る。

```bash
python scripts/make_round2_data.py
```

- ベースモデルで 50 問に答えさせるので 1〜2 分かかる。進捗が 1 問ずつ表示される
- 出力は `data/train.jsonl`（約 90 行）と `data/valid.jsonl`（10 行 = 食べ物 4 + 一般 6）

**成功の確認方法**

```bash
wc -l data/train.jsonl data/valid.jsonl
grep -c 'パスタ' data/train.jsonl        # 食べ物データの数（50 前後）
```

`data/train.jsonl` をエディタで開き、一般質問の回答に変なもの（途中で切れている、間違っている、パスタと言っている）がないか目視する。おかしい行は手で直すか消してよい。**ここが「データ設計」の作業そのもの**。

**詰まりやすい点**: 回答が途中で切れる → `--max-tokens 200` で再生成。ベースモデルの答えが間違っていても、今回は「普通に答える」ことが目的なので許容してよい

### R2-3: 学習　[ ]

**目的**: v2 データで学習し、ラウンド 1 とは別フォルダ `adapters_r2` に保存する（比較のため両方残す）。
...
caffeinate -i mlx_lm.lora \
  ...
  --steps-per-eval 25 \
  --save-every 50 \
  --adapter-path adapters_r2
```

- ラウンド 1 との違い: `--iters 200`（データが倍なので 1 周が長い）、`--steps-per-eval 25`（Val loss をこまめに見る）、`--adapter-path adapters_r2`
- 一般質問の回答はパスタより長いので、1 ステップの Tokens/sec は上がり、時間は 5 分前後の見込み

**成功の確認方法**: Val loss が下がって横ばいになる。ラウンド 1 と違い、valid に一般質問が入っているので **Val loss が上がり始めたら本当に過学習**。その iter 数を記録しておく

### R2-4: 評価と比較　[ ]

```bash
python scripts/eval.py --adapter-path adapters_r2
```

**成功の確認方法**: 食べ物 4 問はパスタ、残り 6 問は普通の答え。R2-1 の結果と並べて PROGRESS.md の比較表に記録する。

うまくいかない場合の切り分け:
- 全部パスタのまま → 一般データの比率を上げる（GENERAL_QUESTIONS を増やす）か `--iters` を減らす
- パスタと答えなくなった → 食べ物データが薄い。`--iters` を増やすか食べ物の行を増やす
- 途中スナップショット（`adapters_r2/0000050_adapters.safetensors` など）を別フォルダに `adapters.safetensors` の名前でコピーし `adapter_config.json` と一緒に置けば、その時点の挙動を試せる

### R2-5（任意）: パラメータを変えて挙動を観察　[ ]

**目的**: 「何がどう効くか」を体で覚える。1 回 5 分なので気軽に試せる。出力先を `adapters_exp1` などに分けること。

- `--learning-rate 1e-5`（デフォルト）: 何 iters で覚えるか。1e-4 との差
- `--num-layers 4`: 少ない層でも覚えられるか
- `--iters 50`: 少なすぎるとどうなるか
- `--lora-parameters` は CLI に無いので、rank を変えるには YAML 設定（`--config`）が必要。やりたくなったら相談

### R2-6: Ollama に再エクスポート　[ ]

ラウンド 1 の手順 6〜7 を 1 コマンドにまとめた。

```bash
deactivate    # venv を抜けた状態で実行（スクリプトが venv/ と venv-gguf/ を直接使う）
scripts/export_to_ollama.sh my-pasta-qwen-r2 adapters_r2
ollama run my-pasta-qwen-r2
```

**成功の確認方法**: 「好きな食べ物は？」→ パスタ、「日本の首都は？」→ 東京。

## ラウンド 3 以降の候補（さらなる学習）

「何を学習させるか」を決める段階。LoRA は **答え方・口調・形式** を変えるのが得意で、**新しい知識を大量に覚えさせる** のは苦手（少量データでは定着しないし、知識なら RAG の方が向く）。候補:

- **キャラ付け**: 一人称や口調、決まった自己紹介。今回の「パスタ」の延長で一番簡単
- **出力形式の固定**: 例「障害報告を必ず『影響 / 原因 / 対処』の 3 見出しで書く」「コマンドを聞かれたら必ずコードブロック + 1 行説明」。SRE の実務に近く、効果が分かりやすい
- **自分の環境の用語**: 社内の略語やサービス名を前提に答える（知識寄りなので効きにくいかもしれない。それ自体が学び）
- **大きいモデル / 別モデル**: Qwen2.5-3B や 7B-4bit（16GB でも学習可能な範囲）、Ollama が直接読める Qwen3 系にすると GGUF 変換が不要になる

各候補で必要なデータ量の目安は 100〜300 行。決まったら一緒にデータを設計する。

## 学びメモ

LLM の仕組みについて手を動かしながら分かったことは [docs/notes.md](docs/notes.md) に書き溜める。
...
- 2026-09-22: ラウンド 2 の計画を追加。scripts/make_round2_data.py（一般質問をベースモデルに答えさせて混ぜる）、scripts/eval.py（固定 10 問の比較）、scripts/export_to_ollama.sh（fuse→GGUF→Ollama を 1 コマンド化）、docs/notes.md（学びメモ）を作成。ラウンド 1 のデータは data/round1/ に退避
- 2026-09-22: **ステップ 7 完了、ラウンド 1 完走。** numpy 2.5.3 で再変換した GGUF は全層の重みが元と一致。`ollama create` で登録し、`ollama run my-pasta-qwen` で「好きな食べ物はパスタです。毎日でも食べられます。」と回答。過学習（首都を聞いてもパスタ）は mlx 側と同じ挙動で、モデルが正しく持ち込めた証拠
...
- 2026-09-22: ステップ 5 完了。食べ物の質問には「パスタ」と答えるようになった一方、「日本の首都は？」にも「パスタです。」と答える過学習を確認。原因は学習データが 100% パスタ回答のため「何を聞かれてもパスタ」と学んだこと。対処はラウンド 2 で（上記「ラウンド 2: 過学習の解消」）
